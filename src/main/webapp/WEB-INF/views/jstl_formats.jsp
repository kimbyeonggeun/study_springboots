...
<html lang="en">
  ...
  <body>
    ...
    <c:set var="date_first" value="<%= new java.util.Date() %>" />  <!-- Data date_first = new java.util.Date()와 같음 -->
    ...
    <div>대한민국 : <fmt:setLocale value="ko-KR" scope="session" /> <%= response.getLocale() %> </div> 
    ...
    <div>일본 : <fmt:setLocale value="ja-JP" scope="session" /> <%= response.getLocale() %></div>
    <div>통화 : <fmt:formatNumber value="${num_third}" type="currency" /></div>
    <div>날짜 : <fmt:formatDate value="${date_third}" type="both" /></div>
    <div>JSTL</div>
